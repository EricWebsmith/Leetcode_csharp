...
namespace Leetcode0017_Recursion
{
    public class Solution
    {
        ...
        private void Recurse(List<char> chars)
        {
            int index = chars.Count;
            if (index >= digits.Length)
            {
                ans.Add(string.Join(string.Empty, chars));
                return;
            }

            foreach(char candidate in mappings[digits[index]])
            {
                List<char> newChars = chars.ToList();
                newChars.Add(candidate);
                Recurse(newChars);
            }
        }
